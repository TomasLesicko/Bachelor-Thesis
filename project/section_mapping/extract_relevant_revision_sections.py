...
def extract_relevant_revision_sections(revision_tags):
    ...
    for tag in revision_tags:
        ...
        subprocess.run("./a.out std.tex > section_names_"+tag, shell=True, check=True)
        subprocess.run("mv section_names_"+tag+" ../../project/section_mapping", shell=True, check=True)
        os.chdir("..")
