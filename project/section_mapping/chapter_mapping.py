...
def read_target_revision_sections(target_tag, current, allow_recursion=True):
    try:
        with open("%s%s.txt" % (chapter_extractor.SECTION_FILE_SHARED_NAME,
                                target_tag), 'r') as sections_text:

            matches = re.findall(SECTIONS_LINE_PARSING_REGEX,
                                 sections_text.read())
...
def map_revision_sections(revision_set, current, older_revision_sections,
                          allow_recursion=True):
    for revision_tag in revision_set:
        try:
            with open("%s%s.txt" % (chapter_extractor.SECTION_FILE_SHARED_NAME,
                                    revision_tag), 'r') as sections_text:

                regex_results = re.findall(SECTIONS_LINE_PARSING_REGEX,
                                           sections_text.read())
            ...
            if allow_recursion:
                print("One or more section_names files could not be found,"
                      " attempting to create them")
                chapter_extractor.extract_relevant_revision_sections(
                    {revision_tag})
                map_revision_sections({revision_tag}, current,
                                      older_revision_sections, False)


def write_mapping(older_revision_sections, target_tag):
    with open("%s%s.json" %
              (SECTION_MAPPING_SHARED_NAME, target_tag), 'w') as sm:
